...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile, error: profErr } = await authClient.from('registration_profiles').select('is_admin').eq('user_id', user.id).single();
    if (profErr || !profile?.is_admin) return new Response('Forbidden', {
      status: 403,
      headers: corsHeaders
    });
    ...
    if (!title || !message) return new Response('Invalid payload', {
      status: 400,
      headers: corsHeaders
    });
    ...
    if (userId) {
      ...
      if (authError) {
        ...
        targetUserIds = (authData?.users || []).map((u)=>u.id);
    ...
    const chunk = (arr, size)=>arr.reduce((acc, _, i)=>i % size ? acc : [
          ...acc,
          arr.slice(i, i + size)
        ], []);
