...
serve(async (req)=>{
  try {
    ...
    // Get all active users - FIXED: removed 'email' column that doesn't exist
    ...
    for (const user of users || []){
      try {
        ...
        const { data: loginCheck, error: loginError } = await adminClient.rpc('check_no_login_notification', {
        ...
        const { data: workoutCheck, error: workoutError } = await adminClient.rpc('check_no_workout_notification', {
          p_user_id: user.user_id
        });
        if (workoutError) {
        ...
        const { data: mealCheck, error: mealError } = await adminClient.rpc('check_no_meal_notification', {
          p_user_id: user.user_id
        });
        if (mealError) {
    ...
    const { data: pendingNotifications, error: notifError } = await adminClient.from('notifications').select('id, user_id, title, message, type').eq('status', 'draft').eq('target_audience', 'user').not('user_id', 'is', null);
    if (notifError) {
    ...
    for (const notification of pendingNotifications || []){
      try {
        ...
        const { data: tokens, error: tokenError } = await adminClient.from('device_tokens').select('expo_token').eq('user_id', notification.user_id);
        ...
        const { error: updateError } = await adminClient.from('notifications').update({
          status: 'sent',
          sent_at: new Date().toISOString()
        }).eq('id', notification.id);
        if (updateError) {
