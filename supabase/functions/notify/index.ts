...
serve(async (req) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceKey) {
    return new Response('Missing function environment: SUPABASE_URL/ANON_KEY/SERVICE_ROLE_KEY', { status: 500 });
  }
    ...
    const { data: profile, error: profErr } = await authClient
      ...
      .eq('user_id', user.id)
      .single();
    if (profErr || !profile?.is_admin) return new Response('Forbidden', { status: 403 });
    ...
    if (!title || !message) return new Response('Invalid payload', { status: 400 });
    ...
    if (tokens.length === 0) return new Response(JSON.stringify({ sent: 0, results: [] }), { status: 200, headers: { 'content-type': 'application/json' } });
    ...
    return new Response(JSON.stringify({ sent: tokens.length, results }), { status: 200, headers: { 'content-type': 'application/json' } });
  } catch (e) {
