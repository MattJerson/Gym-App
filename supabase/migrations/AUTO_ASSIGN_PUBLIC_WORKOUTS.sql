-- Scalable approach: Update get_user_assigned_workouts to include public workouts
-- without requiring explicit assignments
...
BEGIN
  ...
  WHERE wt.assignment_type = 'public'
    AND wt.is_active = true
    AND NOT EXISTS (
      -- Don't duplicate if already explicitly assigned
      SELECT 1 
  ...
  GROUP BY wt.id, wt.name, wt.description, wt.difficulty, wt.duration_minutes, 
           wt.category_id, wc.name, wt.created_at
...
COMMENT ON FUNCTION public.get_user_assigned_workouts IS 'Returns both explicitly assigned workouts and public workouts for a user';
